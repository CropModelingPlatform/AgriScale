...
def ET0pm_Tdew_vectorized(lat, Alt, J, Tn, Tx, Tm, Tdewn, Tdewx, Vm, Rg):
    sigma = 4.903e-9
    ...
    Tdewn = np.asarray(Tdewn, dtype=np.float64)
    Tdewx = np.asarray(Tdewx, dtype=np.float64)
...
def process_chunk(df_chunk, coords):
    ...
    df_chunk["Etppm"] = ET0pm_Tdew_vectorized(
        ...
        df_chunk["Tdewmin"].values,
        df_chunk["Tdewmax"].values,
