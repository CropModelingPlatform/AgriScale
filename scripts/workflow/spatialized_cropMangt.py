...
def main():
    try:
        ...
        print("step 1 ", df_mangt.head(10), flush=True)
        spatialized = False
        ...
        if nc_files.count(None) == len(nc_files):
            print("No spatialized file selected", flush=True)
        else:
            spatialized = True
            print("Selected spatialized files:", flush=True)
            for nc_file in nc_files:
                if nc_file is not None:
                    print(f"- {nc_file}", flush=True)
        ...
        if spatialized_files :
            ...
            print("step 2", nc_spacialized, flush=True)
            if testoption != 1:
                if shapefile == 1:
                    shapefile_path = glob(os.path.join(work_dir,'data', 'shapefile', '*.shp'))[0]
                    gdf = gpd.read_file(shapefile_path)
                ...
                print("step 3", nc_spacialized, flush=True)
                ...
                print("step4", df_spatialized.head(10), flush=True)
            else:
                ...
                ds_spatialized = nc_spacialized.sel(lat =yy, lon=xx, method = "nearest")
                df_spatialized = ds_spatialized.to_dataframe().dropna(axis=0, how="any")
                print("step4", df_spatialized.head(10), flush=True)
            ...
            if s_variety == 1:
                ...
                print("si variete spat", df_spatialized.head(10), flush=True)
                # reset the index
                #df_spatialized = df_spatialized.reset_index(drop=True)
            ...
            import math, gc
            chunk_size = 10000
            total = len(df_spatialized)
            n_chunks = math.ceil(total / chunk_size)
            
            # Sauvegarder df_mangt original pour le produit cartésien
            df_mangt_original = df_mangt.copy()
            df_mangt = None  # Libérer la mémoire
            
            for i, start in enumerate(range(0, total, chunk_size)):
                end = min(start + chunk_size, total)
                chunk = df_spatialized.iloc[start:end].copy()
                
                # Produit cartésien : chaque ligne de df_mangt_original avec ce chunk
                df_temp = df_mangt_original.copy()
                df_temp["key"] = 1
                chunk["key"] = 1
                result_chunk = df_temp.merge(chunk, on="key", how="outer")
                result_chunk.drop("key", axis=1, inplace=True)
                
                # Stocker le résultat
                if i == 0:
                    df_mangt = result_chunk
                else:
                    df_mangt = pd.concat([df_mangt, result_chunk], ignore_index=True)
                
                print(f"Chunk {i+1}/{n_chunks} : lignes {start} à {end} ({len(result_chunk)} lignes dans ce chunk, {len(df_mangt)} total)", flush=True)
                del chunk, df_temp, result_chunk
                gc.collect()
            
            del df_mangt_original
            gc.collect()
            # remove rows without values in Idcultivar column
            #df_mangt = df_mangt.loc[df_mangt['Idcultivar'].notna()].copy()                
            
        
            print(df_mangt.head(10), flush=True)
        ...
        # Supprimer la colonne 'index' si elle existe dans df_mangt
        if 'index' in df_mangt.columns:
            df_mangt = df_mangt.drop(columns=['index'])
